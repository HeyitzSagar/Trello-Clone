...
export default function CardModal({
  ...
  function handleCreateCard() {
    if (cardDescriptionInput.length < 25) {
      const newCard = {
        cardId: v4(),
        ...
        <div>
          <div className="flex flex-wrap gap-2 my-3">
            {cardLabelsArr.map((label) => (
              <span
                key={v4()}
                className={`${label.labelColor} text-white rounded-full p-2 flex gap-1 `}
              >
                <h1>{label.labelText}</h1>
                <button
                  onClick={() => {
                    setCardLabelsArr((prev) =>
            ...
            <button
              ...
              onClick={() => {
                setCardLabelsArr((prev) => [
                  ...
                  {
                    id: v4(),
                    labelText: cardLabelInput,
                    labelColor: currentLabelColor || "bg-gray-500",
                  },
                ]);
                setCardLabelInput("");
              }}
          ...
          <div className="flex gap-6 mt-3">
            <button
              onClick={() => setCurrentLabelColor("bg-blue-500")}
              className={`w-10 h-10 focus:ring-2 focus:ring-blue-500 focus:ring-offset-4  rounded-full p-5 bg-blue-500`}
            ></button>
            <button
              onClick={() => setCurrentLabelColor("bg-red-500")}
              className={`w-10 h-10 focus:ring-2 focus:ring-red-500 focus:ring-offset-4  rounded-full p-5 bg-red-500`}
            ></button>
            <button
              onClick={() => setCurrentLabelColor("bg-green-500")}
              className={`w-10 h-10 focus:ring-2 focus:ring-green-500 focus:ring-offset-4  rounded-full p-5 bg-green-500`}
            ></button>
            <button
              onClick={() => setCurrentLabelColor("bg-gray-500")}
              className={`w-10 h-10 focus:ring-2 focus:ring-gray-500 focus:ring-offset-4  rounded-full p-5 bg-gray-500`}
            ></button>
            <button
              onClick={() => setCurrentLabelColor("bg-yellow-500")}
              className={`w-10 h-10 focus:ring-2 focus:ring-yellow-500 focus:ring-offset-4  rounded-full p-5 bg-yellow-500`}
            ></button>
          </div>
        </div>
